function calcularSoma(): number {
  ...
  console.log(
    `Desafio 2 - O número ${numeroFibonacci} ` +
  ...
  function analisarFaturamento(dados: number[]): {
    menorValor: number;
    maiorValor: number;
  ...
  function calcularPercentualPorEstado(dados: Record<string, number>): Record<string, string> {
  ...
  console.log(
    `Desafio 5 - String invertida: "${stringParaInverter}" -> "${inverterString(
      stringParaInverter
    )}"`
  );
